...
interface Selectors {
    ...
    _getLastClick: (state: RootState) => { index: number; fileId: string } | null;
    ...
    makeGetAction: (
        fileActionSelector: (state: RootState) => string | null
    ) => (state: RootState) => FileAction | null;
    makeGetOptionValue: (
        optionId: string,
        defaultValue: any
    ) => (state: RootState) => any;
    makeGetFiles: (
        fileIdsSelector: (state: RootState) => string[]
